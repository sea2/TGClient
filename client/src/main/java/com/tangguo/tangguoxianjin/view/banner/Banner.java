package com.tangguo.tangguoxianjin.view.banner;
...
public class Banner extends FrameLayout implements ViewPager.OnPageChangeListener {
    ...
    public void setBannerTitle(String[] titles) {
        this.titles = titles;
        if (bannerStyle == BannerConfig.CIRCLE_INDICATOR_TITLE || bannerStyle == BannerConfig.NUM_INDICATOR_TITLE || bannerStyle == BannerConfig.CIRCLE_INDICATOR_TITLE_INSIDE) {
    ...
    public void setImages(List<AdListBean> imagesUrl) {
        setImageList(imagesUrl, null);
    }

    public void setImages(List<AdListBean> imagesUrl, OnLoadImageListener imageListener) {
    ...
    private void setImageArray(Object[] imagesUrl, OnLoadImageListener imageListener) {
        ...
        for (int i = 0; i <= count + 1; i++) {
            ...
            if (imageListener != null) {
                ...
                if (defaultImage != -1) Glide.with(context).load(url).centerCrop().crossFade().into(iv);
                else Glide.with(context).load(url).centerCrop().crossFade().placeholder(defaultImage).into(iv);
            }
        }
        setData();
    }

    private void setImageList(List<AdListBean> imagesUrl, OnLoadImageListener imageListener) {
        ...
        for (int i = 0; i <= count + 1; i++) {
            ...
            Object url = null;
            if (i == 0) {
                url = imagesUrl.get(count - 1).getImgUrl();
            } else if (i == count + 1) {
                url = imagesUrl.get(0).getImgUrl();
            } else {
                url = imagesUrl.get(i - 1).getImgUrl();
            }
            imageViews.add(iv);
            if (imageListener != null) {
                imageListener.OnLoadImage(iv, url);
