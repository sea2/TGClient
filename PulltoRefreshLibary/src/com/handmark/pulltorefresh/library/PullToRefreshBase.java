...
import com.handmark.pulltorefresh.library.internal.XCMLoadingLayout;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout
	...
	public final void onRefreshComplete(String message) {
		if (isRefreshing()) {
			setState(State.RESET, message);
		}
	}

	public final void setHeaderBg(String colornum) {
		if(mHeaderLayout!=null) {
			mHeaderLayout.setHeaderBg(colornum);
		}
	}
	public final void setHeaderTextColor(String colornum) {
		if(mHeaderLayout!=null) {
			mHeaderLayout.setHeaderTextColor(colornum);
		}
	}
	public final void setFooterBg(String colornum) {
		if(mFooterLayout!=null) {
			mFooterLayout.setHeaderBg(colornum);
		}
	}
	...
	final void setState(State state, final boolean... params) {
		...
		if (null != mPullToRefreshStateChangeListener) {
			mPullToRefreshStateChangeListener.pullToRefreshStateChange(mState);
		}
	}

	final void setState(State state, String message, final boolean... params) {
		mState = state;
//		if (!(State.RESET == state)) {
//			mState = state;
//			if (null != mPullToRefreshStateChangeListener) {
//				mPullToRefreshStateChangeListener
//						.pullToRefreshStateChange(mState);
//			}
//		}
		if (DEBUG) {
			Log.d(LOG_TAG, "State: " + mState.name());
	...
	public static interface PullToRefreshStateChangeListener {
		public void pullToRefreshStateChange(State state);
	}

	private PullToRefreshStateChangeListener mPullToRefreshStateChangeListener;

	public void setOnPullToRefreshStateChangeListener(
			PullToRefreshStateChangeListener mPullToRefreshStateChangeListener) {
		this.mPullToRefreshStateChangeListener = mPullToRefreshStateChangeListener;
	}
	...
	protected void onReset(String message) {
		mIsBeingDragged = false;
		mLayoutVisibilityChangesEnabled = true;

		// Always reset both layouts, just in case...
		mHeaderLayout.resetComplete(message);
		mFooterLayout.resetComplete(message);

		// System.out.println("getHandler()" + getHandler());
//		if (null != getHandler()) {
...
//						mPullToRefreshStateChangeListener.pullToRefreshStateChange(mState);
//					}
//					// smoothScrollToLong(0, 200);
//					onReset();
//				}
//			}, 500);
//		}
		smoothScrollTo(0, getPullToRefreshScrollDuration(), 200, new OnSmoothScrollFinishedListener() {
			@Override
			public void onSmoothScrollFinished() {
				mHeaderLayout.reset();
				mFooterLayout.reset();
	...
	protected final void smoothScrollToLong(int scrollValue, long scrollDuration) {
		smoothScrollTo(scrollValue, scrollDuration, 0,
		...
		LoadingLayout createLoadingLayout(Context context, Mode mode,
			...
			case FLIP:
				return new FlipLoadingLayout(context, mode, scrollDirection,
						attrs);
			case NWD:
				return new XCMLoadingLayout(context, mode, scrollDirection,
						attrs);
			}
		}
	}
